...
IMPORTANT:
    ...
    num_envs is supported by solving one independent RMPFlow controller per env.\nCartesian targets are converted from Isaac Lab world coordinates to each\nreplicated environment's local coordinates before being sent to cuMotion.
...
# USD / robot helpers
# -----------------------------------------------------------------------------
def get_env_origin(scene, env_idx=0):
    """Return the Isaac Lab replicated-environment origin in world coordinates."""
    origin = (
...
def get_prim_pose(object01, scene, env_idx=0):
    """
    Read an object's pose and return BOTH world-frame and env-local position.

    Isaac Lab replicates each environment by translating it by scene.env_origins.
    root_link_pose_w is expressed in the global /World frame, while the
    cuMotion robot model is defined with its robot base at the origin of its
    own environment. Therefore Cartesian targets passed to cuMotion must be
    expressed in the corresponding env-local frame.
    """
    ...
    env_origin = get_env_origin(scene, env_idx)
    env_position = world_position - env_origin

    return env_position, quat_xyzw, world_position, env_origin
...
def get_ee_pose(robot, scene, ee_body_id, env_idx=0):
    """
    Return EE pose in both world and replicated-environment-local frames.
    """
    ...
    env_position = world_position - get_env_origin(scene, env_idx)

    return env_position, quat, world_position
...
def main():
    ...
    #
    # IMPORTANT:
    #   root_link_pose_w is GLOBAL /World coordinates.
    #   Each replicated environment has a different scene.env_origins entry.
    #   cuMotion's robot model is local to one robot base at (0, 0, 0), so
    #   Cartesian targets must use:
    #
    #       p_env = p_world - scene.env_origins[env_idx]
    #
    #   Using p_world directly would make env1/env2/... targets appear shifted
    #   by their environment offsets.
    # -------------------------------------------------------------------------
    katao_positions = []
    katao_quats = []
    ...
    for env_idx in range(args_cli.num_envs):
        katao_pos_env, katao_quat, katao_pos_world, env_origin = get_prim_pose(
            katao, scene, env_idx=env_idx
        )
        stick_pos_env, stick_quat, stick_pos_world, _ = get_prim_pose(
            stick_part, scene, env_idx=env_idx
        )

        katao_positions.append(katao_pos_env)
        katao_quats.append(katao_quat)
        stick_part_positions.append(stick_pos_env)
        stick_part_quats.append(stick_quat)

        print(
            f"[ENV {env_idx}] origin={np.round(env_origin, 5)} | "
    ...
    katao_positions = np.stack(katao_positions, axis=0)
    katao_quats = np.stack(katao_quats, axis=0)
    stick_part_positions = np.stack(stick_part_positions, axis=0)
    stick_part_quats = np.stack(stick_part_quats, axis=0)

    initial_ee_pos_env, initial_ee_quat, initial_ee_pos_world = get_ee_pose(
        robot,
        scene,
        ee_body_id,
        env_idx=0,
    ...
    print(f"[INFO] env0 EE world : {np.round(initial_ee_pos_world, 5)}")
    print(f"[INFO] env0 EE local : {np.round(initial_ee_pos_env, 5)}")
    ...
    # -------------------------------------------------------------------------
    # Targets are deliberately built from ENV-LOCAL object coordinates.
    # -------------------------------------------------------------------------
    ...
    for env_idx in range(args_cli.num_envs):
        final_ee_pos_env, _, final_ee_pos_world = get_ee_pose(
            robot,
            scene,
            ee_body_id,
            env_idx=env_idx,
        )
        final_katao_pos_env, _, final_katao_pos_world, _ = get_prim_pose(
            katao,
            scene,
            env_idx=env_idx,
        )

        final_ee_positions.append(final_ee_pos_env)
        final_ee_errors.append(
            float(np.linalg.norm(
                final_ee_pos_env - transport[env_idx]
            ))
        )
        final_katao_positions.append(final_katao_pos_env)
        katao_displacements.append(
            float(np.linalg.norm(
                final_katao_pos_env - katao_positions[env_idx]
            ))
        )

        print(
            f"[ENV {env_idx}] final EE local={np.round(final_ee_pos_env, 5)} | "
            f"world={np.round(final_ee_pos_world, 5)} | "
            f"katao local={np.round(final_katao_pos_env, 5)} | "
            f"world={np.round(final_katao_pos_world, 5)}"
        )
    ...
    for env_idx in range(args_cli.num_envs):
        print(
            ...
            f"target_local={np.round(transport[env_idx], 5)} | "
