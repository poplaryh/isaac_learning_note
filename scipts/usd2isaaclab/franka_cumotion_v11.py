...
parser = argparse.ArgumentParser(
    description="Franka cuMotion RMPFlow independent multi-env pick/place test"
)
...
def get_ee_pose(
    ...
    """
    Return EE pose in both world and environment-local coordinates.
    """
...
def set_gripper_env(
    ...
    """
    Set gripper target for exactly one environment.

    IMPORTANT:
        Do not broadcast this command to all environments.

    During the independent FSM, env0 may already be closing while env1 is
    still approaching. Therefore the command must explicitly specify env_idx.
    """
...
def set_gripper_all(
    ...
    """
    Set the same gripper target for every environment.
...
def main():
    ...
    arm_ids, finger_ids, ee_body_id = get_panda_indices(
        robot
    )
    ...
    # Save the number of completed transitions for diagnostics.
    ...
    def initialize_cartesian_phase(
        ...
        """
        Reset exactly one cuMotion controller for one environment.
        """
    ...
    while simulation_app.is_running():
        ...
        for env_idx in range(
            ...
            # -------------------------------------------------------------
                ...
                # IMPORTANT:
                # Only this environment receives the close command.
            ...
            # -------------------------------------------------------------
                ...
                # IMPORTANT:
                # Only this environment receives the open command.
        ...
        # =====================================================================
        # 3. Evaluate each environment independently
        # =====================================================================
        ...
        for env_idx in range(
            ...
            if current_phase in phase_targets:
                ...
                # Require at least 20 simulation steps before declaring
                # success.
                if (
            ...
            #
            # IMPORTANT:
            # Timeout belongs to the current environment only.
            #
            # If env2 gets stuck in TRANSPORT, env0/env1/env3 continue.
            # -------------------------------------------------------------
        ...
        # =====================================================================
        # 5. Stop only after every environment is DONE or FAILED
        # =====================================================================
    ...
    for env_idx in range(
        ...
        # IMPORTANT:
        # The final Cartesian target is PLACE, not TRANSPORT.
